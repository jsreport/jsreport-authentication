...
function addPassport (reporter, app, admin, definition) {
  ...
  const secret = definition.options.cookieSession && definition.options.cookieSession.secret ? definition.options.cookieSession.secret : reporter.options.encryption.secretKey
  ...
  function authenticate (username, password, done) {
    if (admin.username === username) {
      if (admin.password === password) {
        ...
        return done(null, false, { username, message: 'Invalid password or user does not exist.' })
      }
    }
    ...
    reporter.authentication.usersRepository.authenticate(username, password).then(function (user) {
      if (!user) {
        reporter.authentication.usersRepository.find(username).then((userExists) => {
          ...
          done(null, false, { username, message: 'Invalid password or user does not exist.' })
        })
  ...
  app.post('/login', bodyParser.urlencoded({extended: true, limit: '2mb'}), (req, res, next) => {
    ...
    passport.authenticate('local', (err, user, info) => {
      ...
      if (!user) {
        reporter.authentication.usersRepository.checkFailedLoginAttempts(admin.username === info.username ? admin : {
          ...
          if (validationResult.valid) {
            ...
            req.session.viewModel.login = validationResult.message
          }
      ...
      reporter.authentication.usersRepository.checkFailedLoginAttempts(user).then((validationResult) => {
...
module.exports = function (reporter, definition) {
  ...
  if (
    ...
    reporter.options.encryption.secretKey == null &&
    (
      !definition.options.cookieSession ||
      (definition.options.cookieSession && !definition.options.cookieSession.secret)
    )
  ) {
    throw new Error('A secret is needed for cookie session, fill either "encryption.secretKey" option or "extensions.authentication.cookieSession.secret" option in config')
  }
