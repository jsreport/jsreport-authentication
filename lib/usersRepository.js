...
module.exports = (reporter) => {
  ...
  return {
    ...
    async checkFailedLoginAttempts (currentUser, { validLogin = false } = {}) {
      ...
      if (isInRange) {
        ...
        if (newFailedAttemptsCount >= maxFailedAttempts) {
          ...
          result = { valid: false, message: `Max attempts to login has been reached (${maxFailedAttempts}), login for this user has been locked for ${timeStr}` }
          newFailedAttemptsCount = maxFailedAttempts
        } else {
          result = { valid: true }
        }
      } else {
